...
compile and Run:
g++ -fopenmp PR-testcode1.cpp `pkg-config --cflags --libs opencv`
./a.out image_name seed1 seed2 num_threads
...
int Graph::getMaxFlow(int s, int t, int thread_count)
{
    ...
    #pragma omp parallel for schedule(static) reduction(+:netFlowOutS) num_threads(thread_count)
...
int main(int argc, char** argv)
{
	    ...
	    int seed1 = stoi(argv[2]);
	    int seed2 = stoi(argv[3]);
   ...
   // create the adj-mat
	...
	g.addEdge( 0 , seed1 , max);					
	g.addEdge( seed1 , 0 , max);
	g.addEdge( seed2 , V-1 , max);
	g.addEdge( V-1, seed2 , max);
	...
	*/
	...
	gettimeofday(&t2, 0);
	double time = (1000000.0*(t2.tv_sec-t1.tv_sec) + t2.tv_usec-t1.tv_usec)/1000.0; // Time taken by Recursive doubling Algorithm
