...
int Graph::getMaxFlow(int s, int t, int thread_count)
{
    ...
    // OpenMP Parallel_Push_Relabel Algorithm based on He and Bo's Idea
    // Loop until the convergence criteria is met
    while( ( ver[s].e_flow + ver[t].e_flow ) < TotalExcess )	
    {
	
    		// step 1: break up vertices in chunks. Each thread take responsibility for one particular chunk
			...
			// step 2: Each thread finds the vertex u such that e(u) > 0 in its assigned chunk of vertices.
			int hasexcess = -1;
			...
			// step 3: Call Push_Relabel_Kernel
			...
			// ****Push_Relabel_Kernel Begins ****
		      ...
		      while( cycles > 0 )			// while cycle > 0 do
		      {
			   ...
			   if (hasexcess != -1 )			// if e(u) > 0
				...
				// Each thread finds the vertex u such that e(u) > 0 in its assigned chunk of vertices.
		   		hasexcess = -1;
    				for (int i = startVx; i < endVx ; i++){
					if ((i == s) || (i == t)) continue;
					if (ver[i].e_flow > 0){
...
int main(int argc, char** argv)
{
   ...
   // add the edge to the adjmat
	...
	// ******** Call Parallel Push-Relabel Algorithm ********
